...
public:	
    ...
    DXRR(HWND hWnd, int Ancho, int Alto)
	{
		...
		arriaba = 0;
		billCargaFuego();
		camara = new Camara(D3DXVECTOR3(0,80,6), D3DXVECTOR3(0,80,0), D3DXVECTOR3(0,.5f,0), Ancho, Alto);
		...
		bodega = new ModeloRR(d3dDevice, d3dContext, "Assets/Bodega/Bodega.obj", L"Assets/Bodega/Bodega.png", L"Assets/NoSpecMap.jpg", -65, -80);
		...
		estacionGas = new ModeloRR(d3dDevice, d3dContext, "Assets/EstacionGas/EstacionGas.obj", L"Assets/EstacionGas/EstacionGas.bmp", L"Assets/NoSpecMap.jpg", 77, -77);
		moneda = new ModeloRR(d3dDevice, d3dContext, "Assets/Moneda/Moneda.obj", L"Assets/Moneda/Moneda.png", L"Assets/NoSpecMap.jpg", 20, -98);
		...
		pozo = new ModeloRR(d3dDevice, d3dContext, "Assets/Pozo/Pozo.obj", L"Assets/Pozo/Pozo.jpg", L"Assets/NoSpecMap.jpg", 46, -12);
		torreAgua = new ModeloRR(d3dDevice, d3dContext, "Assets/TorreAgua/TorreAgua.obj", L"Assets/TorreAgua/TorreAgua.bmp", L"Assets/NoSpecMap.jpg", -54, -18);
		LlantasViejas = new ModeloRR(d3dDevice, d3dContext, "Assets/LlantasViejas/LlantasViejas.obj", L"Assets/LlantasViejas/LlantasViejas.bmp", L"Assets/NoSpecMap.jpg", 0, 0);
		
		hCameraPosLabel = CreateWindowEx(0, L"STATIC", L"Posición de la cámara: X=0, Y=0, Z=0",
	...
	void Render(void)
	{
		...
		estacionGas->Draw(camara->vista, camara->proyeccion, terreno->Superficie(77, -77), camara->posCam, 10.0f, 0, 'A', 1);
		moneda->Draw(camara->vista, camara->proyeccion, terreno->Superficie(20, -98), camara->posCam, 10.0f, 0, 'A', 1);
		piedra->Draw(camara->vista, camara->proyeccion, terreno->Superficie(36, 67), camara->posCam, 10.0f, 0, 'A', 1);
		pozo->Draw(camara->vista, camara->proyeccion, terreno->Superficie(46, -12), camara->posCam, 10.0f, 0, 'A', 1);
		torreAgua->Draw(camara->vista, camara->proyeccion, terreno->Superficie(-54, -18), camara->posCam, 10.0f, 0, 'A', 1);
		LlantasViejas->Draw(camara->vista, camara->proyeccion, terreno->Superficie(0, 0), camara->posCam, 10.0f, 0, 'A', 1);

		std::wstringstream ss;
		ss << L"Posición de la cámara: X=" << camara->posCam.x << L", Y=" << camara->posCam.y << L", Z=" << camara->posCam.z;
	...
	void HandleInput(WPARAM wParam)
	{
		...
		case VK_F1:
			...
			ss << L"Posición de la cámara: X=" << camara->posCam.x << L", Y=" << camara->posCam.y << L", Z=" << camara->posCam.z;
			MessageBox(hWnd, ss.str().c_str(), L"Posición de la cámara", MB_OK | MB_ICONINFORMATION);
			break;
		default:
			break;
		}
